...
function wrapPage({ title, description, canonicalPath, bodyClass, mainClass, jsonLd, content }) {
...
function renderTopicGuidePage(page, siblings) {
  const siblingLinks = siblings.map((item) => ({ href: toPath(item.slug), label: item.h1 }));
  const related = [
    { href: toPath(page.pillar.slug), label: `Return to pillar: ${page.pillar.h1}` },
    ...siblingLinks,
    { href: page.tool.href, label: page.tool.label },
    { href: '/study/navle/practice/', label: 'Try 5 free practice questions on this topic' },
...
function renderToolGuidePage(page, siblings) {
  ...
  const inputRows = page.inputs
    .map((row) => `                        <tr><td>${row.name}</td><td>${row.definition}</td></tr>`)
    .join('\n');
...
function renderPillarPage(pillar, supportPages, extraLinks) {
  const supportList = supportPages
    .map((page) => ({ href: toPath(page.slug), label: page.h1, desc: page.summary }))
    .map((item) => `                <article class="pc-bridge-card"><h3><a href="${item.href}">${item.label}</a></h3><p>${item.desc}</p></article>`)
    .join('\n');
...
const emergencySupports = [
  {
    ...
    summary: 'Rapid triage, decompression priorities, shock resuscitation, and surgical timing for suspected GDV.',
    ...
    tool: { href: '/tools/fluid-calculator.html', label: 'Use the Fluid Calculator for shock aliquots' },
    pillar: emergencyPillar,
    primaryQuery: 'gdv approach dog',
    secondaryQueries: ['dog bloat emergency protocol', 'gdv stabilization steps', 'gdv first hour management'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Differentiate shock phenotypes quickly and choose first-line stabilization actions.',
    kicker: 'NAVLE Emergency Guide',
    intro:
    ...
    tool: { href: '/tools/fluid-calculator.html', label: 'Use the Fluid Calculator for perfusion-guided boluses' },
    pillar: emergencyPillar,
    primaryQuery: 'types of shock in veterinary medicine',
    secondaryQueries: ['veterinary shock algorithm', 'hypovolemic vs cardiogenic shock dog', 'shock treatment steps vet'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Recognize sepsis risk, start early support, and avoid delayed source-control decisions.',
    kicker: 'NAVLE Emergency Guide',
    intro:
    ...
    tool: { href: '/tools/sepsis-bundle-planner.html', label: 'Use the Sepsis Bundle Planner' },
    pillar: emergencyPillar,
    primaryQuery: 'sepsis veterinary approach',
    secondaryQueries: ['sirs criteria dog cat', 'sepsis bundle veterinary', 'septic shock treatment vet'],
    paa: [
  ...
  },
  {
    ...
    summary: 'First-hour DKA sequence with fluid-first logic and potassium-aware insulin planning.',
    kicker: 'NAVLE Emergency Guide',
    intro:
    ...
    tool: { href: '/tools/insulin-cri-planner.html', label: 'Use the Insulin CRI Planner' },
    pillar: emergencyPillar,
    primaryQuery: 'dka approach dog cat',
    secondaryQueries: ['veterinary dka treatment sequence', 'insulin cri dka vet', 'dka potassium correction dog'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Recognize heat injury early and run a controlled cooling plus perfusion workflow.',
    kicker: 'NAVLE Emergency Guide',
    intro:
    ...
    tool: { href: '/tools/er-algorithms.html', label: 'Open the ER Algorithms Library' },
    pillar: emergencyPillar,
    primaryQuery: 'heatstroke treatment dog veterinary',
    secondaryQueries: ['controlled cooling heatstroke dogs', 'heatstroke complications vet', 'heatstroke triage protocol'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Core transfusion decision flow for emergency and inpatient veterinary settings.',
    kicker: 'NAVLE Emergency Guide',
    intro:
    ...
    tool: { href: '/tools/transfusion-helper.html', label: 'Use the Transfusion Helper' },
    pillar: emergencyPillar,
    primaryQuery: 'veterinary transfusion basics',
    secondaryQueries: ['dog cat blood transfusion indications', 'transfusion reactions veterinary', 'packed rbc vs plasma vet'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Handle suspected anticoagulant rodenticide bleeding with structured triage and monitoring.',
    kicker: 'NAVLE Emergency Guide',
    intro:
    ...
    tool: { href: '/tools/toxin-decontamination-planner.html', label: 'Use the Toxin Decontamination Planner' },
    pillar: emergencyPillar,
    primaryQuery: 'rodenticide bleeding dog treatment',
    secondaryQueries: ['anticoagulant rodenticide vet approach', 'coagulopathy dog poison triage', 'rodenticide hemorrhage management'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Differentiate pleural disease causes and prioritize respiratory stabilization steps.',
    kicker: 'NAVLE Emergency Guide',
    intro:
    ...
    tool: { href: '/tools/chf-staging-planner.html', label: 'Use the CHF Staging Planner' },
    pillar: emergencyPillar,
    primaryQuery: 'pleural effusion differentials dog cat',
    secondaryQueries: ['vet pleural effusion workup', 'thoracocentesis indications dog cat', 'pleural effusion vs edema'],
    paa: [
...
const cardiologySupports = [
  {
    ...
    summary: 'Stage CHF cases with practical pattern cues and follow-up timing logic.',
    ...
    secondaryQueries: ['veterinary chf stages', 'dog chf follow-up intervals', 'feline chf staging'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Structured murmur triage and differential narrowing across species and life stages.',
    kicker: 'Cardiology NAVLE Guide',
    intro:
    ...
    secondaryQueries: ['veterinary murmur workup', 'systolic murmur differential dog', 'feline murmur triage'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Differentiate common high-yield tachyarrhythmias and match first-step response.',
    kicker: 'Cardiology NAVLE Guide',
    intro:
    ...
    tool: { href: '/tools/emergency-drug-chart.html', label: 'Use the Emergency Drug Chart' },
    pillar: cardiologyPillar,
    primaryQuery: 'ecg rhythm identification af svt vt veterinary',
    secondaryQueries: ['vet af vs svt', 'vt treatment dog cat', 'tachyarrhythmia triage veterinary'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Connect blood pressure severity to organ risk and follow-up strategy.',
    kicker: 'Cardiology NAVLE Guide',
    intro:
    ...
    tool: { href: '/tools/chf-staging-planner.html', label: 'Use the CHF Staging Planner for follow-up structure' },
    pillar: cardiologyPillar,
    primaryQuery: 'systemic hypertension target organ damage dog cat',
    secondaryQueries: ['feline hypertension tod', 'canine hypertension management', 'vet blood pressure monitoring'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Core DCM pattern recognition, diagnostic flow, and monitoring strategy.',
    kicker: 'Cardiology NAVLE Guide',
    intro:
    ...
    secondaryQueries: ['dilated cardiomyopathy veterinary', 'dog dcm workup', 'feline dcm recognition'],
    paa: [
  ...
  },
  {
    ...
    title: 'Pleural Effusion vs Pulmonary Edema in Veterinary Patients – VetLudics',
    description:
    ...
    summary: 'Differentiate two common dyspnea patterns and adjust first-step interventions.',
    kicker: 'Cardiology NAVLE Guide',
    intro:
    ...
    secondaryQueries: ['dyspnea differential dog cat', 'thoracocentesis vs diuretics', 'cardiogenic edema veterinary'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Medication role framing, monitoring priorities, and stage-aware adjustment concepts.',
    kicker: 'Cardiology NAVLE Guide',
    intro:
    ...
    secondaryQueries: ['chf medications dog cat', 'diuretic monitoring veterinary', 'pimobendan educational guide'],
    paa: [
  ...
  },
  {
    ...
    summary: 'Differentiate syncopal causes and prioritize cardiovascular versus non-cardiac workup.',
    kicker: 'Cardiology NAVLE Guide',
    intro:
    ...
    secondaryQueries: ['collapse differential veterinary', 'cardiogenic syncope dog', 'feline syncope workup'],
    paa: [
...
const calculatorSupports = [
  {
    ...
    summary: 'Core mg/kg formula logic with concentration conversion safety checks.',
    intro:
    ...
    inputs: [
      { name: 'Body weight', definition: 'Patient weight in kg (or converted from lb accurately).' },
      { name: 'Dose target', definition: 'Intended mg/kg dose selected for the clinical objective.' },
      { name: 'Drug concentration', definition: 'Product concentration in mg/mL or equivalent.' }
    ],
    outputs: [
      { name: 'Total mg required', definition: 'Calculated medication amount before concentration conversion.' },
      { name: 'Administration volume', definition: 'Final mL value for administration planning.' }
    ],
    formulaSummary:
    ...
    referenceLink: { href: '/dog-cat-normal-values/', label: 'Cross-check lab context in normal values reference' },
    pillar: calculatorPillar,
    primaryQuery: 'mg kg dosing guide veterinary',
    secondaryQueries: ['how to calculate mg/kg dose dog cat', 'veterinary dose conversion', 'mg/ml conversion vet'],
    paa: ['How do you calculate mg/kg dose?', 'How do you convert mg to mL?', 'What dosing mistakes are most common?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Plan CRI concentration and infusion rate with practical setup safeguards.',
    intro:
    ...
    inputs: [
      { name: 'Patient weight', definition: 'Weight in kilograms for dose normalization.' },
      { name: 'CRI target dose', definition: 'Prescribed rate in mcg/kg/min or mg/kg/hr.' },
      { name: 'Prepared concentration', definition: 'Final concentration in infusion bag or syringe.' }
    ],
    outputs: [
      { name: 'Infusion rate', definition: 'Pump setting in mL/hr.' },
      { name: 'Dose verification', definition: 'Back-calculated dose to confirm setup accuracy.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/cri-calculator.html', label: 'Use the CRI Calculator' },
    topicLinks: [
      { href: '/dka-approach-veterinary/', label: 'Apply CRI sequence in DKA stabilization' },
      { href: '/sepsis-sirs-veterinary/', label: 'See infusion priorities in sepsis support' },
      { href: '/shock-types-veterinary/', label: 'Compare infusion choices across shock types' }
    ],
    referenceLink: { href: '/reference/normal-values.html', label: 'Reference monitoring ranges during CRI therapy' },
    pillar: calculatorPillar,
    primaryQuery: 'cri setup guide veterinary',
    secondaryQueries: ['veterinary infusion rate calculation', 'mcg kg min to ml hr', 'cri dose check'],
    paa: ['How do you set up a CRI in dogs and cats?', 'How do you convert CRI dose to mL/hr?', 'What are common CRI safety errors?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Convert dehydration estimates into replacement plans with reassessment checkpoints.',
    intro:
    ...
    inputs: [
      ...
      { name: 'Estimated dehydration', definition: 'Clinical dehydration percentage estimate.' },
      { name: 'Replacement window', definition: 'Target timeframe for deficit replacement.' }
    ],
    outputs: [
      { name: 'Deficit volume', definition: 'Estimated mL needed to replace dehydration deficit.' },
      { name: 'Hourly replacement target', definition: 'Deficit portion translated to mL/hr over selected window.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/fluid-calculator.html', label: 'Use the Fluid Calculator' },
    topicLinks: [
      { href: '/shock-types-veterinary/', label: 'Link deficit planning with shock phenotype decisions' },
      { href: '/heatstroke-veterinary/', label: 'Apply fluid logic in heatstroke stabilization' },
      { href: '/gdv-approach/', label: 'Use deficit logic in GDV perfusion planning' }
    ],
    referenceLink: { href: '/dog-cat-normal-values/', label: 'Use normal values to interpret fluid response trends' },
    pillar: calculatorPillar,
    primaryQuery: 'fluid deficit calculation veterinary',
    secondaryQueries: ['dehydration deficit dog cat', 'vet fluid replacement formula', 'how to calculate fluid deficit'],
    paa: ['How do you calculate fluid deficit in dogs and cats?', 'How fast should fluid deficit be replaced?', 'How do ongoing losses change fluid plans?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Calculate baseline maintenance rates and adjust with practical monitoring logic.',
    intro:
    ...
    inputs: [
      { name: 'Body weight', definition: 'Weight in kilograms used for maintenance estimation.' },
      { name: 'Clinical status', definition: 'Hydration/perfusion context that may modify baseline maintenance.' },
      { name: 'Concurrent losses', definition: 'Vomiting, diarrhea, drains, or polyuria affecting net need.' }
    ],
    outputs: [
      { name: 'Maintenance estimate', definition: 'Baseline mL/day or mL/hr requirement.' },
      { name: 'Adjusted delivery rate', definition: 'Practical rate after losses and tolerance are considered.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/fluid-calculator.html', label: 'Use the Fluid Calculator' },
    topicLinks: [
      { href: '/dka-approach-veterinary/', label: 'Apply maintenance logic during DKA stabilization phases' },
      { href: '/sepsis-sirs-veterinary/', label: 'See maintenance adjustment in sepsis reassessment cycles' },
      { href: '/pleural-effusion-vs-pulmonary-edema/', label: 'Balance fluids in cardiopulmonary compromise cases' }
    ],
    referenceLink: { href: '/reference/normal-values.html', label: 'Monitor lab trends while adjusting maintenance rates' },
    pillar: calculatorPillar,
    primaryQuery: 'maintenance fluid calculation veterinary',
    secondaryQueries: ['vet maintenance fluids dog cat', 'ml kg day conversion', 'maintenance fluid rate mL hr'],
    paa: ['How are maintenance fluids calculated?', 'How do you convert mL/day to mL/hr?', 'When should maintenance rates be adjusted?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Compute and interpret anion gap in clinical context with caveats.',
    intro:
    ...
    inputs: [
      ...
      { name: 'Bicarbonate/Total CO2', definition: 'Buffer component used in anion gap calculation.' }
    ],
    outputs: [
      { name: 'Anion gap', definition: 'Calculated estimate of unmeasured anion burden.' },
      { name: 'Interpretive pattern', definition: 'Contextual clue for high-anion vs normal-anion metabolic acidosis pathways.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/acid-base-electrolyte.html', label: 'Use the Acid-Base and Electrolyte Tool' },
    topicLinks: [
      { href: '/dka-approach-veterinary/', label: 'Apply anion gap logic in DKA interpretation' },
      { href: '/sepsis-sirs-veterinary/', label: 'Use acid-base context in sepsis progression checks' },
      { href: '/shock-types-veterinary/', label: 'Pair perfusion status with acid-base interpretation' }
    ],
    referenceLink: { href: '/dog-cat-normal-values/', label: 'Compare values with species normal ranges' },
    pillar: calculatorPillar,
    primaryQuery: 'anion gap interpretation veterinary',
    secondaryQueries: ['anion gap formula dog cat', 'high anion gap acidosis vet', 'acid base calculator veterinary'],
    paa: ['How is anion gap calculated?', 'What causes high anion gap in dogs and cats?', 'Can normal anion gap still mean serious disease?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Understand osmolality math, interpretation limits, and clinical use cases.',
    intro:
    ...
    inputs: [
      { name: 'Sodium', definition: 'Major contributor to extracellular osmolality.' },
      { name: 'Glucose', definition: 'Important contributor in hyperglycemic states.' },
      { name: 'BUN', definition: 'Can influence calculated osmolality context.' }
    ],
    outputs: [
      { name: 'Calculated osmolality', definition: 'Estimated plasma osmolality for pattern assessment.' },
      { name: 'Interpretive context', definition: 'Framework for fluid-shift and neurologic-risk reasoning.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/acid-base-electrolyte.html', label: 'Use the Acid-Base and Electrolyte Tool' },
    topicLinks: [
      { href: '/dka-approach-veterinary/', label: 'Connect osmolality reasoning to DKA fluid planning' },
      { href: '/heatstroke-veterinary/', label: 'Integrate osmolality context in severe dehydration states' },
      { href: '/shock-types-veterinary/', label: 'Use osmolality trends in perfusion-guided reassessment' }
    ],
    referenceLink: { href: '/reference/normal-values.html', label: 'Compare electrolytes with reference intervals' },
    pillar: calculatorPillar,
    primaryQuery: 'osmolality basics veterinary',
    secondaryQueries: ['calculated osmolality dog cat', 'osmolar gap veterinary', 'hyperosmolar interpretation vet'],
    paa: ['How is osmolality calculated in veterinary patients?', 'When should measured osmolality be used?', 'Why does osmolality matter in DKA?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Plan dextrose adjustments safely during hypoglycemia and insulin workflows.',
    intro:
    ...
    inputs: [
      { name: 'Current glucose', definition: 'Measured glucose at decision point.' },
      { name: 'Target glucose range', definition: 'Goal range based on clinical context and protocol.' },
      { name: 'Available dextrose concentration', definition: 'Stock concentration used for dilution planning.' }
    ],
    outputs: [
      { name: 'Dextrose plan', definition: 'Practical infusion or bolus approach aligned to target correction.' },
      { name: 'Monitoring cadence', definition: 'Frequency for repeat glucose and reassessment checks.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/electrolyte-replacement-planner.html', label: 'Use the Electrolyte Replacement Planner' },
    topicLinks: [
      { href: '/dka-approach-veterinary/', label: 'Apply dextrose transitions in DKA management' },
      { href: '/sepsis-sirs-veterinary/', label: 'Monitor glucose stability in septic critical care cases' },
      { href: '/heatstroke-veterinary/', label: 'Integrate glucose support in severe heat injury care' }
    ],
    referenceLink: { href: '/dog-cat-normal-values/', label: 'Reference glucose ranges in dogs and cats' },
    pillar: calculatorPillar,
    primaryQuery: 'dextrose correction guide veterinary',
    secondaryQueries: ['hypoglycemia treatment dog cat', 'dextrose infusion calculation vet', 'glucose correction veterinary'],
    paa: ['How do you correct hypoglycemia in dogs and cats?', 'How often should glucose be checked during correction?', 'How is dextrose dilution calculated?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Practical BCS interpretation linked to nutrition and follow-up planning.',
    intro:
    ...
    inputs: [
      { name: 'Species and body type', definition: 'Dog/cat context and breed conformation.' },
      { name: 'Palpation findings', definition: 'Rib cover, waist, and abdominal tuck observations.' },
      { name: 'Current intake pattern', definition: 'Feeding routine, treats, and activity profile.' }
    ],
    outputs: [
      { name: 'BCS category', definition: 'Underconditioned, ideal, or overconditioned classification.' },
      { name: 'Nutrition planning direction', definition: 'Caloric adjustment framework and follow-up target range.' }
    ],
    formulaSummary:
    ...
    referenceLink: { href: '/reference/normal-values.html', label: 'Pair BCS with baseline lab interpretation trends' },
    pillar: calculatorPillar,
    primaryQuery: 'body condition score guide dog cat',
    secondaryQueries: ['veterinary bcs chart', 'dog cat body condition assessment', 'bcs interpretation vet'],
    paa: ['How is body condition score assessed?', 'What BCS is ideal for dogs and cats?', 'How often should BCS be rechecked?'],
    intent: 'informational'
  },
  {
    ...
    summary: 'Estimate exposure burden and triage urgency with safer interpretation steps.',
    intro:
    ...
    inputs: [
      { name: 'Estimated amount ingested', definition: 'Best available quantity estimate from owner history.' },
      { name: 'Patient weight', definition: 'Weight in kilograms for dose normalization.' },
      { name: 'Substance concentration', definition: 'Active compound amount per tablet, mL, or gram.' }
    ],
    outputs: [
      { name: 'Estimated mg/kg exposure', definition: 'Dose estimate used for risk framing.' },
      { name: 'Triage urgency category', definition: 'Initial urgency interpretation before definitive toxicology consult.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/toxin-decontamination-planner.html', label: 'Use the Toxin Decontamination Planner' },
    topicLinks: [
      { href: '/rodenticide-bleeding-veterinary/', label: 'Apply toxicity math in rodenticide bleeding cases' },
      { href: '/heatstroke-veterinary/', label: 'Compare toxin triage to other emergency stabilization workflows' },
      { href: '/sepsis-sirs-veterinary/', label: 'Differentiate toxic collapse from infectious instability patterns' }
    ],
    referenceLink: { href: '/reference/toxicities.html', label: 'Cross-check toxicology reference profiles' },
    pillar: calculatorPillar,
    primaryQuery: 'toxic dose calculation veterinary',
    secondaryQueries: ['dog cat toxicity mg kg calculation', 'poison dose estimate vet', 'toxicology triage calculations'],
    paa: ['How do you calculate toxic dose in dogs and cats?', 'What if ingested amount is uncertain?', 'When should toxicity cases be treated immediately?'],
    intent: 'tool'
  },
  {
    ...
    summary: 'Reference-style guide to lab variability, red flags, and escalation decisions.',
    intro:
    ...
    inputs: [
      { name: 'Current lab values', definition: 'CBC, chemistry, electrolytes, and related diagnostics.' },
      { name: 'Species and signalment', definition: 'Dog/cat differences plus age and breed context.' },
      { name: 'Clinical timeline', definition: 'Acute versus chronic trajectory and treatment phase.' }
    ],
    outputs: [
      { name: 'Contextual interpretation', definition: 'Prioritized meaning of abnormalities in current case context.' },
      { name: 'Escalation decision', definition: 'When to monitor, intervene, or urgently re-evaluate.' }
    ],
    formulaSummary:
    ...
    tool: { href: '/tools/unit-converter.html', label: 'Use the Unit Converter for lab normalization' },
    topicLinks: [
      { href: '/dog-cat-normal-values/', label: 'Reference dog and cat normal value ranges' },
      { href: '/anion-gap-interpretation/', label: 'Interpret acid-base shifts with anion gap context' },
      { href: '/pleural-effusion-differentials-veterinary/', label: 'Use lab context in respiratory differential prioritization' }
    ],
    referenceLink: { href: '/sources-and-limitations/', label: 'Review general sources and limitations policy' },
    pillar: calculatorPillar,
    primaryQuery: 'veterinary lab interpretation caveats',
    secondaryQueries: ['dog cat lab variability', 'veterinary red flag labs', 'how to interpret lab trends vet'],
    paa: ['How should veterinary labs be interpreted safely?', 'What lab patterns are emergency red flags?', 'Why do species differences matter in lab interpretation?'],
    intent: 'informational'
  }
];

function siblingsFor(group, currentIndex) {
  return [group[(currentIndex + 1) % group.length], group[(currentIndex + 2) % group.length]];
}
...
writePage(
  ...
  renderPillarPage(emergencyPillar, emergencySupports, [
    { href: '/tools/er-algorithms.html', label: 'Use the ER algorithms tool for rapid reinforcement' },
    { href: '/study/navle/practice/', label: 'Try 5 free emergency-focused practice questions' },
    { href: '/veterinary-calculators-guide/', label: 'Open the calculators guide for emergency math support' },
    { href: '/pricing/', label: 'See Premium benefits for unlimited emergency practice sets' }
  ])
);

emergencySupports.forEach((page, index) => {
  writePage(page.slug, renderTopicGuidePage(page, siblingsFor(emergencySupports, index)));
});

writePage(
  cardiologyPillar.slug,
  renderPillarPage(cardiologyPillar, cardiologySupports, [
    { href: '/cardiology-chf-algorithm/', label: 'Open the CHF algorithm landing page' },
    { href: '/tools/chf-staging-planner.html', label: 'Use the CHF staging tool' },
    { href: '/study/navle/practice/', label: 'Try 5 free cardiology practice questions' },
    { href: '/pricing/', label: 'Review Premium benefits for unlimited cardiology practice' }
  ])
);

cardiologySupports.forEach((page, index) => {
  writePage(page.slug, renderTopicGuidePage(page, siblingsFor(cardiologySupports, index)));
});
...
writePage(
  ...
  renderPillarPage(calculatorPillar, calculatorSupports, [
    ...
    { href: '/veterinary-calculators/', label: 'Use the veterinary calculators hub' },
    { href: '/study/navle/practice/', label: 'Test calculation concepts in practice questions' },
    { href: '/pricing/', label: 'Review Premium benefits for unlimited concept practice' }
  ])
);

calculatorSupports.forEach((page, index) => {
  writePage(page.slug, renderToolGuidePage(page, siblingsFor(calculatorSupports, index)));
});
...
function buildKeywordRow(pageType, page, links) {
  ...
  const secondary = page.secondaryQueries ? page.secondaryQueries.join('; ') : 'cluster hub query set';
...
function addClusterKeywordRows(pillar, supports, pillarLinkSet) {
  ...
  supports.forEach((page, index) => {
    const siblings = siblingsFor(supports, index).map((item) => toPath(item.slug));
    const links = [
      toPath(pillar.slug),
      page.tool.href,
      ...siblings,
