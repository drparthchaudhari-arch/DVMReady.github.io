(function () {
    ...
    async function loadLeaderboard() {
        ...
        try {
            if (!isSupabaseReady()) {
                showEmptyState('Leaderboard unavailable in local mode. Log in to participate.');
                return;
            }

            var supabase = getSupabaseClient();
            if (!supabase) {
                showEmptyState('Leaderboard client unavailable. Please refresh and try again.');
                return;
            }
            ...
            if (!data.length) {
                showEmptyState('No scores yet. Be the first to complete a case study!');
                return;
            }
    ...
    function isSupabaseReady() {
        ...
        var configured = typeof window.isSupabaseConfigured === 'function'
    ...
    async function showUserRank(supabase) {
        ...
        try {
            ...
            if (userRankDiv) {
                userRankDiv.innerHTML = '<p>You are ranked <strong>#' + rank + '</strong> this week.</p>';
            }
        } catch (error) {
