(function () {
    ...
    function renderRows(rows) {
        ...
        if (!rows || !rows.length) {
            body.innerHTML = '<tr><td colspan="5">No scores yet. Complete a case study to appear here.</td></tr>';
            return;
        }
    ...
    function renderUserRank(rank) {
        ...
        if (!rank) {
            node.textContent = 'Complete more case studies to get ranked.';
            return;
        }
    ...
    async function loadLeaderboard() {
        ...
        if (!isConfigured()) {
            renderRows([]);
            setStatus('Sync not configured yet. Leaderboard will appear after setup.', false);
            return;
        }
        ...
        if (!client) {
            renderRows([]);
            setStatus('Supabase client unavailable. Check configuration scripts.', true);
            return;
        }
        ...
        try {
            var response = await client
                .from('leaderboard')
                .select('user_id, display_name, total_completions, current_streak, weekly_score, updated_at')
                .order('weekly_score', { ascending: false })
                .order('updated_at', { ascending: false })
                .limit(10);
            ...
            if (!rows.length) {
                ...
                setStatus('Scores update when you complete case studies.', false);
            }
