(function () {
    ...
    function calculateChocolate(weight, hours, contraindicated) {
        var chocolateType = String(document.getElementById('tox-chocolate-type').value || 'milk');
        var amountOz = toNumber(document.getElementById('tox-chocolate-amount').value);
        var typeData = CHOCOLATE[chocolateType] || CHOCOLATE.milk;

        if (!Number.isFinite(amountOz) || amountOz < 0) {
            return {
                ok: false,
                alert: { level: 'danger', text: 'Enter a valid chocolate amount in ounces.' }
            };
        }
        ...
        var monitoring = 'Home monitoring may be acceptable if asymptomatic and low dose.';
        var alert = { level: 'caution', text: 'Call a veterinarian if clinical signs begin.' };

        if (doseMgKg >= 60) {
            risk = 'Severe-Lethal Range';
            action = 'Emergency treatment now: decontamination, charcoal, fluids, arrhythmia/seizure support.';
            monitoring = 'Continuous ECG/neurologic monitoring likely required.';
            alert = { level: 'danger', text: 'Critical methylxanthine dose estimate.' };
        } else if (doseMgKg >= 40) {
            ...
            alert = { level: 'danger', text: 'Dose likely to cause significant clinical signs.' };
        } else if (doseMgKg >= 20) {
            risk = 'Mild-Moderate';
            action = 'Prompt vet guidance recommended. Decontamination may still help.';
        ...
        return {
            ...
            monitoring: monitoring + ' Approximate half-life in dogs is prolonged (about 17.5h).',
            alert: contraindicated
                ? { level: 'danger', text: 'Contraindication flag present: avoid emesis without clinician confirmation.' }
                : alert
        };
    }

    function calculateXylitol(weight, hours, contraindicated) {
        var product = String(document.getElementById('tox-xylitol-product').value || 'gum');
        ...
        var mgPerUnit = XYLITOL_PRODUCTS[product] ? XYLITOL_PRODUCTS[product].mgPerUnit : NaN;

        if (product === 'custom') {
            mgPerUnit = custom;
        }

        if (!Number.isFinite(amount) || amount < 0 || !Number.isFinite(mgPerUnit) || mgPerUnit <= 0) {
            return {
                ok: false,
                alert: { level: 'danger', text: 'Enter valid xylitol amount and mg-per-unit values.' }
            };
        }
        ...
        var risk = 'Low';
        var action = 'Monitor closely for weakness, vomiting, tremors, and lethargy.';
        var monitoring = 'If signs develop, immediate blood glucose testing is required.';
        var alert = { level: 'caution', text: 'Xylitol can trigger rapid insulin release.' };

        if (doseGKg >= 0.5) {
            risk = 'High (Hepatic Injury Risk)';
            action = 'Emergency hospitalization advised. Decontamination and aggressive hepatic monitoring needed.';
            monitoring = 'Serial glucose, ALT/AST, bilirubin, coagulation profile over 72h.';
            alert = { level: 'danger', text: 'Dose above hepatotoxic threshold (>= 0.5 g/kg).' };
        } else if (doseGKg >= 0.1) {
            ...
            alert = { level: 'danger', text: 'Dose above hypoglycemia threshold (>= 0.1 g/kg).' };
        }
    ...
    function calculateRodenticide(weight, hours, contraindicated) {
        var rodenticideType = String(document.getElementById('tox-rodenticide-type').value || 'warfarin');
        var amountGrams = toNumber(document.getElementById('tox-rodenticide-amount').value);
        var concentrationPercent = toNumber(document.getElementById('tox-rodenticide-percent').value);
        var typeData = RODENTICIDES[rodenticideType] || RODENTICIDES.warfarin;

        if (!Number.isFinite(amountGrams) || amountGrams < 0 || !Number.isFinite(concentrationPercent) || concentrationPercent <= 0) {
            return {
                ok: false,
                alert: { level: 'danger', text: 'Enter valid rodenticide bait amount and concentration percent.' }
            };
        }
        ...
        var risk = 'Low';
        var action = 'Discuss decontamination and home monitoring plan with veterinarian.';
        var alert = { level: 'caution', text: 'Any rodenticide ingestion warrants veterinary consultation.' };

        if (ratioToToxic >= 2) {
            risk = 'High';
            action = 'Emergency treatment recommended now. Start targeted tox protocol.';
            alert = { level: 'danger', text: 'Estimated dose exceeds 2x listed toxic threshold.' };
        } else if (ratioToToxic >= 1) {
            risk = 'Moderate';
            action = 'Urgent decontamination and directed monitoring advised.';
            alert = { level: 'danger', text: 'Estimated dose is at or above listed toxic threshold.' };
        }
    ...
    function render(event) {
        ...
        if (!Number.isFinite(weight) || weight <= 0 || !Number.isFinite(hours) || hours < 0) {
            setAlert({ level: 'danger', text: 'Enter valid weight and hours-since-ingestion inputs.' });
    ...
    function init() {
        ...
        for (var i = 0; i < reactiveInputs.length; i += 1) {
            if (reactiveInputs[i].id === 'tox-mode' || reactiveInputs[i].id === 'tox-xylitol-product') {
                continue;
            }
