(function () {
    ...
    function collectCaseCompletions() {
        ...
        if (completions && typeof completions === 'object' && Object.keys(completions).length) {
            return completions;
        }
            ...
            for (var i = 0; i < localStorage.length; i += 1) {
                ...
                if (key && key.indexOf('pc_case_') === 0 && localStorage.getItem(key) === 'completed') {
                    ...
                    fallback[key.replace('pc_case_', '')] = {
                        completedAt: localStorage.getItem(dateKey) || new Date().toISOString()
                    };
                }
            }
        } catch (error) {
    ...
    function getStreakValue() {
        ...
        if (window.PCStorage && typeof window.PCStorage.getStreak === 'function') {
            ...
            return Number.isFinite(Number(storageStreak.current)) ? Number(storageStreak.current) : 0;
        }
    ...
    function displayAccountData() {
        ...
        if (caseList) {
            if (completedCount > 0) {
                caseList.innerHTML = completedKeys.map(function (key) {
                    var item = completions[key];
                    var completedAt = item && (item.completedAt || item.completed_at || item.date);
                    var date = completedAt ? new Date(completedAt).toLocaleDateString() : 'Unknown date';
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', displayAccountData, { once: true });
    } else {
        displayAccountData();
    }
})();
