(function () {
    ...
    function compensationInterpretation(primary, pCO2, hco3) {
        ...
        if (primary.indexOf('Metabolic Acidosis') !== -1) {
            ...
            var high = expected + 2;
            if (pCO2 < low) {
                return 'Expected pCO2 ' + format(expected, 1) + ' (Winter). Actual lower than expected: concurrent respiratory alkalosis possible.';
            }
            if (pCO2 > high) {
                return 'Expected pCO2 ' + format(expected, 1) + ' (Winter). Actual higher than expected: concurrent respiratory acidosis possible.';
            }
        ...
        if (primary.indexOf('Metabolic Alkalosis') !== -1) {
            ...
            return 'Expected pCO2 about ' + format(capped, 1) + ' for metabolic alkalosis compensation.';
        }
        ...
        if (primary.indexOf('Respiratory Acidosis') !== -1) {
            ...
            return 'Expected HCO3: acute ' + format(acute, 1) + ', chronic ' + format(chronic, 1) + ' mEq/L.';
        }
        ...
        if (primary.indexOf('Respiratory Alkalosis') !== -1) {
            ...
            return 'Expected HCO3: acute ' + format(acuteAlk, 1) + ', chronic ' + format(chronicAlk, 1) + ' mEq/L.';
        }
    ...
    function calculate(event) {
        ...
        var species = String(document.getElementById('ab-species').value || 'canine');
        ...
        if (Number.isFinite(pH) && (pH < 6.8 || pH > 7.8)) {
            alertMessage = 'Input pH appears outside physiologic range. Recheck analyzer values.';
            alertLevel = 'danger';
        }

        var primary = getPrimaryDisorder(pH, pCO2, hco3);

        var anionGap = Number.isFinite(na) && Number.isFinite(cl) && Number.isFinite(hco3)
            ? na - (cl + hco3)
            : NaN;

        var normalAlbumin = species === 'feline' ? 2.8 : 3.3;
        var correctedAG = Number.isFinite(anionGap) && Number.isFinite(albumin)
            ? anionGap + (2.5 * (normalAlbumin - albumin))
            : NaN;

        var deltaRatio = Number.isFinite(correctedAG) && Number.isFinite(hco3) && (24 - hco3) > 0
            ? (correctedAG - 12) / (24 - hco3)
            : NaN;

        var baseExcess = Number.isFinite(pH) && Number.isFinite(hco3)
        ...
        setText('ab-cag', format(correctedAG, 1) + (Number.isFinite(correctedAG) ? ' mEq/L' : ''));
        setText('ab-delta', format(deltaRatio, 2));
        setText('ab-be', format(baseExcess, 1) + (Number.isFinite(baseExcess) ? ' mEq/L' : ''));
        ...
        if (Number.isFinite(osmolality)) {
            setText('ab-osm', 'Estimated osmolality: ' + format(osmolality, 1) + ' mOsm/kg (2 x Na + glucose/18 + BUN/2.8).');
            if (!alertMessage && osmolality > 330) {
                alertMessage = 'Marked hyperosmolar estimate detected. Correlate clinically and confirm with measured osmolality if available.';
                alertLevel = 'caution';
            }
        } else {
        ...
        if (!alertMessage && primary.indexOf('Mixed') !== -1) {
            alertMessage = 'Mixed acid-base pattern suspected. Full blood gas and clinical context are required before treatment decisions.';
            alertLevel = 'caution';
        }
