(function () {
        ...
        if (species === 'cat') {
            if (upc < 0.4) {
                return {
                    ...
                    action: 'Repeat UPC and blood pressure to verify persistence before major plan changes.'
                };
            }
            ...
            return {
                ...
                action: 'Confirm persistence and discuss proteinuria-targeted renal management with clinician protocol.'
            };
        }
    ...
    function getBpCategory(sbp, repeats, hasTodConcern) {
        if (!Number.isFinite(sbp) || sbp <= 0) {
            return {
                ...
                recheck: 'Collect calm serial blood pressure readings before classing risk.'
            };
        }
        ...
        if (hasTodConcern) {
            ...
            recheck = 'Urgent same-day workup for ocular, neurologic, and renal injury patterns.';
        } else if (repeats < 3 && sbp < 180) {
            recheck += ' Obtain at least 3 readings in a calm setting before final class assignment.';
        }
    ...
    function getRenalPattern(flags, stage) {
        ...
        if (ckdScore >= 2 && akiScore >= 1) {
            return {
                ...
                action: 'Treat as unstable renal course and reassess values, urine output, and perfusion frequently.'
            };
        }
        ...
        if (akiScore >= 2 && ckdScore <= 1) {
            return {
                ...
                action: 'Escalate AKI workup and perfusion-directed stabilization without delay.'
            };
        }
        ...
        if (ckdScore >= 2 && akiScore === 0) {
            return {
                ...
                action: 'Use CKD staging/substaging trends to drive interval monitoring and long-term plan adjustments.'
            };
        }
        ...
        if (akiScore === 1 && ckdScore <= 1) {
            return {
                ...
                action: 'Repeat chemistry and urine output checks soon to separate transient change from evolving injury.'
            };
        }
        ...
        return {
            ...
            action: 'Integrate trend data, urinalysis, imaging, and hemodynamic context before final pattern labeling.'
        };
    }
    ...
    function render(event) {
        ...
        if (!Number.isFinite(creatinine) || creatinine <= 0) {
            setText('kap-note', 'Enter a valid creatinine value to generate renal stage output.');
        ...
        var flags = {
            ...
            acuteTrigger: !!(byId('kap-acute-trigger') && byId('kap-acute-trigger').checked),
        ...
        if (stage.stage >= 3) {
            actions.push('Stage 3-4 range detected: prioritize close electrolyte, acid-base, hydration, and anemia trend surveillance.');
        } else {
            actions.push('Track serial renal trends (creatinine/SDMA/UPC/BP) to detect progression before overt decompensation.');
        }

        if (bp.level === 'hypertensive' || bp.level === 'severely_hypertensive' || flags.tod) {
            actions.push('Blood pressure concern present: include retinal and neurologic checks in immediate reassessment workflow.');
        }
